...
Для общей истории приложение не чаще раза в сутки обращается к `raw.githubusercontent.com`. Обновления приложения загружаются с этого же хоста; GitHub Releases используется как запасной источник. Запросы обновления могут содержать версию приложения, ОС и архитектуру, необходимые Velopack для выбора пакета; идентификаторы пользователя и содержимое локального календаря не передаются. Загрузку общей истории можно отключить в `Справка` → `О программе`; уже загруженные записи при этом остаются локально.
