...
## Эксплуатация и перенос VPS

Рабочий runner называется `taskbot-msk-marking-calendar`. После переноса VPS
восстановлен 10.09.2026: пользователь `actions-runner`, каталог
`/opt/marking-calendar-runner`, служба systemd с включённым автозапуском.
Проверочный [запуск 10.09.2026](https://github.com/jadieify-hub/marking-calendar/actions/runs/34464797299)
завершился успешно: источник вернул `UNCHANGED`, карта групп синхронизирована.
Параметры SSH конкретного рабочего окружения хранятся в локальном файле
`docs/self-hosted-runner.local.md`, исключённом из Git. Ключи и токены в репозиторий
не добавляются.

Проверка службы на сервере:

```bash
cd /opt/marking-calendar-runner
sudo ./svc.sh status
systemctl is-enabled "$(cat .service)"
sudo journalctl -u "$(cat .service)" -n 50 --no-pager
```

Если служба остановлена, запустите `sudo ./svc.sh start`, затем проверьте состояние
`Idle` или `Active` в GitHub и результат ожидающего задания. Статус `queued` без
назначенного runner означает, что проверка источника ещё не выполнялась.

При следующем переносе VPS отдельно перенесите или заново зарегистрируйте runner:
копирования каталогов и контейнеров ботов для этого недостаточно. Остановите старую
службу, установите runner на новом сервере по инструкции выше и при регистрации
с прежним именем используйте `--replace`. Завершите перенос только после успешного
запуска `Public calendar history` на новом сервере.
...
curl --silent --show-error --output /dev/null \
  --write-out '%{http_code}\n' \
  --user-agent 'MarkingCalendar/0.1.12' \
